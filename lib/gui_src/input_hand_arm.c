...
static void handleEvent_quit(int signo)
{
	LOG(LOG_INFO,"in qq handle sig %d \n", signo);    
	pthread_exit(NULL);
	close_device(gUiDataAll.sInput, PRINT_DEVICE_ERRORS | PRINT_DEVICE | PRINT_DEVICE_NAME);
	free(ufds);
}

static void *get_EventMsg(void *args)  
{
    int i,max;
    int index;
    int res;
	...
	signal(SIGQUIT,handleEvent_quit);
	
    nfds = 1;
    ufds = calloc(1, sizeof(ufds[0]));
    ufds[0].fd = inotify_init();
    ufds[0].events = POLLIN;
	res = inotify_add_watch(ufds[0].fd, gUiDataAll.sInput, IN_DELETE | IN_CREATE);
    if(res < 0) {
        fprintf(stderr, "could not add watch for %s, %s\n", gUiDataAll.sInput, strerror(errno));
		free(ufds);
        return NULL;
    }
    res = scan_dir(gUiDataAll.sInput, PRINT_DEVICE_ERRORS | PRINT_DEVICE | PRINT_DEVICE_NAME);
    if(res < 0) {
        fprintf(stderr, "scan dir failed for %s\n", gUiDataAll.sInput);
		free(ufds);
        return NULL;
    }
	//printf("----nfds[%d]optind[%d]-input[%s]----\r\n",nfds,optind,gUiDataAll.sInput);

    while(1) 
	{
		poll(ufds, nfds, -1);
		if(ufds[0].revents & POLLIN) {
		   read_notify(gUiDataAll.sInput, ufds[0].fd, PRINT_DEVICE_ERRORS | PRINT_DEVICE | PRINT_DEVICE_NAME);
		}
        for(index = 1; index < nfds; index++) 
		{
            if(!(ufds[index].revents & POLLIN))
				continue;
            res = read(ufds[index].fd, event, sizeof(event));
...
{
	int rc;
	rc = pthread_create(&key_thread, NULL, get_EventMsg,NULL);
	if (rc)
	{
...
void Stop_HandInput(void)
{
	...
	LOG(LOG_INFO,"Stop HandInput thread %d\n", ret);
