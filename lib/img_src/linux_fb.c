...
static ret_t linux_fb_set(fb_info_t* fb, uint32_t x, uint32_t y, const rgba_t* rgba) {
  ...
  // *p = pixel;
		r=rgba->r*rgba->a + p->r*(256-rgba->a);
		g=rgba->g*rgba->a + p->g*(256-rgba->a);
		b=rgba->b*rgba->a + p->b*(256-rgba->a);
		p->r=r/256;
		p->g=g/256;
		p->b=b/256;
		p->a=rgba->a;
...
ret_t linux_fb_from_bitmap(fb_info_t* fb, bitmap_t* b) {
  uint32_t xsta,xend,x;
  uint32_t ysta,yend,y;
  return_value_if_fail(fb != NULL && b != NULL, RET_BAD_PARAMS);
  ...
  for (y = 0; y < yend; y++) {
    for (x = 0; x < xend; x++) {
      ...
      linux_fb_set(fb, xsta+x, ysta+y, &rgba);
    }
  }
